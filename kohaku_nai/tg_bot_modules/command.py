...
DrawCommand = Alconna(
    ...
    Args["prompt", str],
    Option(
        "--negative_prompt", Args.negative_prompt[str], help_text="Set Negative Prompt"
    ),
    Option("--seed", Args["seed", int], help_text="Set Seed Of Generation"),
    Option(
        "--steps",
        Args["steps", Annotated[int, lambda x: x < 70]],
        help_text="Set Steps Of Generation",
    ),
...
if __name__ == "__main__":
    ...
    # BAD CASE
    dov = DrawCommand.parse(
        "/draw aaaaa -nefg aaaa -s 123 -st 123 -cfg 123 -sam k_dpmpp_2m -wi 123 -he 123"
    )
    print(dov)
